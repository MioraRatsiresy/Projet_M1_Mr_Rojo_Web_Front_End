...
export class ListComponent implements OnInit {
    ...
    this.listeService.getListRdv().subscribe({
      next: (res) => {
        ...
        this.calendarOptions.events =
          this.listeService.convertRdvToCalendarEvent(
            this.listRdv,
            this.services
          );
  ...
  onSelectService(id: Event) {
    this.rdv.idService = this.idServiceChoosed = (
      id.target as HTMLInputElement
    ).value;
  }
  ...
  onAdd() {
    ...
    this.rdv.prix = this.services.find(
      (service) => service.id == this.rdv.idService
    ).prix;
    this.rdv.dateHeureDebut = datechoosed.toISOString();
    datechoosed.setMinutes(
      datechoosed.getMinutes() +
        this.services.find((service) => service.id == this.rdv.idService).delai
    );
    ...
    this.offresSpec.forEach((element) => {
      if (this.rdv.idService == element.idService) {
        //	console.log();
        if (
          new Date(this.rdv.dateHeureDebut) >= new Date(element.dateDebut) &&
          new Date(this.rdv.dateHeureDebut) <= new Date(element.dateFin)
        ) {
          this.rdv.prix =
            this.rdv.prix - (this.rdv.prix * element.reduction) / 100;
        }
      }
    });
    this.http
      .post(base_url + '/clients/rdv/insert', { rdv: this.rdv })
      .subscribe(
        (res: any) => {
          // Handle successful response
          if (res) {
  ...
  async getListRdv(): Promise<any> {
    try {
      ...
      if (res) {
        ...
        this.calendarOptions.events =
          this.convertRdvToCalendarEvent(responseData);
        //console.log(responseData);
  ...
  convertRdvToCalendarEvent(rdvs: any[]): any[] {
    ...
    rdvs.forEach((item) => {
      const convertedItem = {
        title: this.getServiceName(item.idService),
        start: item.dateHeureDebut,
        end: item.dateHeureFin,
        idRdv: item.id,
      };
  ...
  async deleteRdv(idRdv: string) {
    ...
    try {
      const res = await this.http
        .delete(base_url + '/clients/rdv/delete/', { params })
        .toPromise();
      //	console.log(res);
      this.getListRdv();
    } catch (error) {
  ...
  async handleEventClick(info: any) {
    ...
    if (result) {
      ...
      if (info.event.start > new Date()) {
        ...
        const indexToRemove = this.listRdv.findIndex(
          (rdv) => rdv.id === info.event.extendedProps.idRdv
        );
  ...
  async getListHoraire() {
    const res: any = await this.http
      .get(base_url + '/clients/horaire/list/')
      .toPromise();
    if (res) {
      // Check if res contains data directly or nested under a property
      const responseData = res.hasOwnProperty('data') ? res['data'] : res;
