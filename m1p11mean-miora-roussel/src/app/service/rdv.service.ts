...
export class RdvService {
  private apiUrl = 'http://localhost:8080'; // Assurez-vous que l'URL correspond à votre API
  ...
  cancelRdv(id: string): Observable<any> {
    return this.http.get<any>(`${this.apiUrl}/api_employee/valider_un_rdv?id=${id}&status=-10`).pipe(
  ...
  indisponibiliseRdv(dateDebut: Date, dateFin: Date, idEmploye: string): Observable<any> {
    ...
    return this.http.post<any>(`${this.apiUrl}/api_employee/inserer_rdv`, body).pipe(
      catchError(this.handleError)
    );
  }
  ...
  private handleError(error: HttpErrorResponse): Observable<never> {
    if (error.error instanceof ErrorEvent) {
      ...
      // Erreur côté serveur
      console.error(
        `Code d'erreur de l'API ${error.status}, ` +
        `body: ${error.error}`);
